...
def test_create_edge_rejects_global_node(monkeypatch):
    ...
    try:
        ...
        wb = Node(sort_order=0, work_id=work.id, type="worldbuilding", title="世界观", scope="global")
        ch = Node(sort_order=0, work_id=work.id, type="chapter", title="章节")
        db.add_all([wb, ch])
        db.commit()
...
def test_create_edge_rejects_element_to_non_chapter(monkeypatch):
    ...
    try:
        work = _make_work(monkeypatch, db)
        elem = Node(sort_order=0, work_id=work.id, type="element", title="觉醒")
        vol = Node(sort_order=0, work_id=work.id, type="volume", title="卷")
        db.add_all([elem, vol])
        db.commit()
...
def test_create_edge_rejects_protagonist_character(monkeypatch):
    ...
    try:
        work = _make_work(monkeypatch, db)
        protagonist = Node(sort_order=0, work_id=work.id, type="character", title="主角", scope="global")
        ch = Node(sort_order=0, work_id=work.id, type="chapter", title="章节")
