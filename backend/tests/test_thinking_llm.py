...
def test_stream_falls_back_when_primary_only_returns_empty_chunks():
    primary = FakeLLM(
        "p",
        stream_chunks=[AIMessageChunk(content=""), AIMessageChunk(content="")],
    )
    fallback = FakeLLM("f", stream_chunks=[AIMessageChunk(content="备用回复")])

    chunks = list(FallbackLLM(primary, fallback).stream("x"))

    assert [chunk.content for chunk in chunks] == ["", "", "备用回复"]


def test_stream_accepts_reasoning_only_chunk_as_meaningful():
    primary = FakeLLM(
        "p",
        stream_chunks=[
            AIMessageChunk(
                content="",
                additional_kwargs={"reasoning_content": "正在思考"},
            )
        ],
    )
    fallback = FakeLLM("f", stream_chunks=[AIMessageChunk(content="不应调用")])

    chunks = list(FallbackLLM(primary, fallback).stream("x"))

    assert len(chunks) == 1
    assert chunks[0].additional_kwargs["reasoning_content"] == "正在思考"


def test_stream_accepts_tool_call_chunk_as_meaningful():
    primary = FakeLLM(
        "p",
        stream_chunks=[
            AIMessageChunk(
                content="",
                tool_call_chunks=[
                    {"name": "query_nodes", "args": "{}", "id": "call-1", "index": 0}
                ],
            )
        ],
    )
    ...
    assert len(chunks) == 1
    assert chunks[0].tool_call_chunks[0]["name"] == "query_nodes"
...
def test_astream_falls_back_when_primary_only_returns_empty_chunks():
    primary = FakeLLM(
        "p",
        astream_chunks=[AIMessageChunk(content=""), AIMessageChunk(content="")],
    )
    fallback = FakeLLM(
        "f",
        astream_chunks=[AIMessageChunk(content="备用回复")],
    )
    fb = FallbackLLM(primary, fallback)

    async def _run():
        out = []
        async for chunk in fb.astream("x"):
            out.append(chunk.content)
        return out

    assert asyncio.run(_run()) == ["", "", "备用回复"]
...
def test_get_llm_propagates_extra_body(monkeypatch):
    ...
    assert type(llm).__name__ != "ThinkingChatOpenAI"
    primary = getattr(llm, "_primary", llm)
    assert isinstance(primary, NovelLLM)
    assert primary._extra_body == {"thinking": {"type": "adaptive"}}
    assert isinstance(primary.provider, object)
    assert primary.provider.config.model == "m"
...
def test_get_llm_propagates_extra_body_to_fallback(monkeypatch):
    ...
    assert fb._primary.provider.config.name == "p"
    assert fb._fallback.provider.config.name == "f"
