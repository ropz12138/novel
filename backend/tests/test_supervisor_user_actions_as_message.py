...
def test_load_chat_history_excludes_consumed_actions_message(mock_auth):
    db = database.SessionLocal()
    try:
        session = session_store.create_session(user_id=mock_auth.id)
        sid = session["id"]
        ...
        session_store.add_message(sid, "user", "编辑后的普通消息")
    finally:
        db.close()

    history, current_turn = SupervisorAgent()._load_chat_history(sid)

    assert history == []
    assert [message["content"] for message in current_turn] == [
        "编辑后的普通消息",
    ]
...
def test_start_stores_actions_message_before_user_text(mock_auth, monkeypatch):
    ...
    assert "event: user_actions_message_stored" in resp.text
    assert '我删除了「第6章」节点。' in resp.text
    ...
    try:
        ...
        assert '我删除了「第6章」节点。' in last_two[0].content
        assert "碎片" not in last_two[0].content
...
def test_resume_streams_actions_message_before_user_text(mock_auth, monkeypatch):
    db = database.SessionLocal()
    try:
        work = CanvasWork(user_id=mock_auth.id, title="已有作品")
        db.add(work)
        db.commit()
        db.refresh(work)
        ...
        node = Node(
            ...
            type="chapter",
            title="第7章",
            content="修改后的正文不会进入消息",
        )
        db.add(node)
        db.commit()
        db.refresh(node)
        svc.record_node_action(
            db,
            work_id=work.id,
            user_id=mock_auth.id,
    ...
    monkeypatch.setattr(
        sup_router,
        "_capture_checkpoint_before_agent",
        lambda *a, **k: None,
    )
    monkeypatch.setattr(sup_router, "SessionLocal", database.SessionLocal)
    ...
    assert resp.status_code == 200
    assert resp.text.index("event: user_actions_message_stored") < resp.text.index(
        "event: user_message_stored"
    )
    assert '我修改了「第7章」节点。' in resp.text
    assert "修改后的正文不会进入消息" not in resp.text
