...
async def start_supervisor(
    ...
    async def event_generator():
        task = _launch_supervisor_task(
            queue=queue,
            runner=lambda agent: agent.start(message=payload.message),
...
def _launch_supervisor_task(
    *,
    ...
    def _persist_event_message(db: Session, session_id: str, event: str, data: dict):
        from app.models.agent_model import SupervisorSession
        ...
        if event == "stage_start":
            ...
            if stage not in {"chapter_write", "edit_chapter", "evaluation"}:
                return
    ...
    def emit(event: str, data: dict):
        ...
        if current_session_id and run_db is not None and isinstance(data, dict):
            try:
                if event in {"stage_start", "evaluation_done", "edit_chapter_diff", "edit_chapter_auto_applied"}:
                    _persist_event_message(run_db, current_session_id, event, data)
            except Exception:
