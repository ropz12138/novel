...
from app.services.agents.tools import get_outline_agent_tools
...
def set_outline_context(context: Dict[str, Any]):
    """设置当前上下文"""
    global _current_context
    _current_context = context


def get_outline_context() -> Dict[str, Any]:
    """获取当前上下文"""
    return _current_context


class OutlineAgentState(MessagesState):
    """大纲Agent状态"""
    user_message: str = ""
    canvas_overview: str = ""
...
class OutlineAgent:
    """大纲Agent - 使用LangGraph StateGraph编排"""
    ...
    def _build_system_prompt(self, user_message: str, canvas_overview: str) -> str:
        """构建系统提示"""
        template_path = PROMPT_DIR / "outline_agent_system.txt"
    ...
    def _build_graph(self):
        """构建LangGraph"""
        tools = get_outline_agent_tools()
        tool_node = ToolNode(tools)

        llm = get_llm(temperature=0.7)
        llm_with_tools = bind_tools_to_llm(llm, tools)

        async def agent_node(state: OutlineAgentState):
            """Agent节点"""
            messages = state["messages"]

            if self.emit:
                await self.emit("thinking", {"status": "大纲Agent思考中..."})
    ...
    async def run(self, user_message: str, canvas_overview: str = "", work_id: str = None) -> dict:
        ...
        try:
            # 设置上下文
            set_outline_context({"work_id": work_id})
            ...
            logger.info("OutlineAgent cancelled (client disconnected)")
            return {"success": False, "error": "cancelled"}

        except Exception as e:
            logger.error(f"OutlineAgent error: {e}")
            return {
                "success": False,
                "error": str(e),
            }
