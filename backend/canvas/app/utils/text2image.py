...
def generate_and_save(api_key: str, prompt: str, save_path: Path, size: str | None = None) -> None:
    ...
    if provider == "ark":
        from app.utils.ark_image import generate_and_save as ark_generate_and_save

        ark_generate_and_save(api_key, prompt, save_path, effective_size)
        return
    if provider == "dashscope":
        from app.utils.bailian_image import generate_and_save as dashscope_generate_and_save
