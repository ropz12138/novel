...
from app.node_types import validate_edge_endpoints, validate_character_relation
...
def test_validate_relation_type_rejects_empty():
    from app.node_types import validate_relation_type
    import pytest
...
def test_create_character_relation_tool_skips_existing_pair_with_warning(monkeypatch):
    import importlib
    rt = importlib.import_module("app.services.agents.tools.character_relation_tools")
    db = database.SessionLocal()
    try:
...
def test_list_update_delete_character_relation(mock_auth):
    db = database.SessionLocal()
    try:
        work = _make_work(db, mock_auth.id)
        a = _make_character(db, work.id, "A")
        ...
        updated = client.put(
            ...
            json={"relation_type": "生死之交", "label": "第三卷决裂前"},
        )
        assert updated.status_code == 200
        assert updated.json()["relation_type"] == "生死之交"
