...
from app.services.supervisor_event_persist import (
    PERSISTABLE_TODO_EVENTS,
    enrich_messages_with_todolist,
    persist_supervisor_event,
)
from app.services.todo_service import update_todolist, write_todolist
...
def test_persist_task_status_updated_mutates_existing_card(db_session):
    ...
    items = db_session.query(__import__("app.models.todo_item", fromlist=["TodoItem"]).TodoItem).filter_by(session_id=session_id).all()
    db_id = items[0].id
