...
覆盖：
- record_node_action / record_edge_action：记录字段正确（create/delete 存 preview，update 不存）
- has_substantial_node_change：节点 PUT 降噪（仅 position/locked 不记）
- build_pending_actions_section：水位线过滤 + 增删给全 / 改引导查询
...
def test_record_node_create_stores_content_preview(work_and_user):
    ...
    assert "林远在仓库醒来" in a.content_preview
    assert a.content_preview != ""


def test_record_node_delete_stores_preview_before_loss(work_and_user):
    db, work, user = work_and_user
    ...
    assert a.target_title == "第一章"
    assert "被删节点的内容" in a.content_preview
...
# ---------- build_pending_actions_section（水位线过滤 + 增删给全/改引导） ----------
...
def test_build_section_includes_all_when_watermark_none(work_and_user):
    ...
    assert "新建" in section
    assert "主角" in section
    assert "觉醒" in section  # create 含 preview
...
def test_build_section_update_guides_to_query_not_inlining_content(work_and_user):
    db, work, user = work_and_user
    ...
    assert "第三章" in section
    assert "read_node_content" in section  # 引导语
    assert "非常长的修改后正文" not in section  # 不内联内容
