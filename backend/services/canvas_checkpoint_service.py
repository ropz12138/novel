...
def prepare_edit_resend(
    ...
    *,
    ...
    """编辑重发：恢复画布并重建被编辑的用户消息。

    与原消息紧邻的操作摘要保留为历史，因为其操作已体现在 checkpoint 中，
    但不再提供给 Agent；checkpoint 之后的操作记录属于废弃分支并一并删除。
    """
    msg = (
        db.query(SupervisorMessage)
    ...
    if (msg.meta or {}).get("type") == "user_canvas_actions":
        raise ValueError("用户操作消息不可编辑")
    ...
    previous = (
        ...
        .filter(
            SupervisorMessage.session_id == session_id,
            SupervisorMessage.sort_order < msg.sort_order,
        )
    ...
    if (
        ...
        and (previous.meta or {}).get("type") == "user_canvas_actions"
    ):
        previous.meta = {
            **(previous.meta or {}),
    ...
    (
        ...
        .filter(
            UserCanvasAction.work_id == work_id,
            UserCanvasAction.created_at > checkpoint.created_at,
        )
        .delete(synchronize_session=False)
    )
    db.commit()
    truncate_messages_from(db, session_id, truncate_from_order)
