...
from services.todo_service import list_todo_items, serialize_todo_item
...
def _persist_diff_event(db: Session, session_id: str, event: str, data: dict) -> bool:
    """将节点 content diff 事件写入 supervisor_messages。"""
    sess = db.query(SupervisorSession).filter_by(id=session_id).first()
    if not sess:
        return False

    if event in ("chapter_edit_diff", "node_content_diff"):
        diff = data.get("diff") or {}
        node_id = data.get("node_id") or data.get("chapter_node_id") or ""
        db.add(
            SupervisorMessage(
                id=str(uuid.uuid4()),
                ...
                meta={
                    ...
                    "chapterContentDiffCard": {
                        "node_id": node_id,
                        "chapter_node_id": data.get("chapter_node_id") or node_id,
